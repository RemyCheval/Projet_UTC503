<?php 
  ...
  $sqlArticles = "SELECT MIN(joueur.idClub) AS mini, MAX(joueur.idClub) AS maxi, MIN(club.idClub) AS mini, MAX(club.idClub) AS maxi INNER JOIN joueur ON joueur.idClub = club.idClub FROM club;";
  ...
  if($idArt < $miniArt || $idArt > $maxiArt) {
    ...
    $articles = $conn->query("SELECT * FROM club INNER JOIN joueur ON joueur.idClub = club.idClub WHERE idClub = $idArt;");
